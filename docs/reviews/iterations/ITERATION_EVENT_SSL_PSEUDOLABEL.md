...
- **55683040 Public = 0.145（较最佳回退 0.001）**。
...
| > 0.146 | 新基线 | 未发生 |
| = 0.146 | 保留研究 | 未发生 |
| **< 0.146（0.145）** | **伪标签有害，冻结** | ✅ |

### 机制分析

- OOF 单模 +0.0225 的巨大提升源于**逼近 ensemble**：PL 微调把模型输出推向伪标签（ensemble 预测）；在 train 域验证集上 ensemble 本身与真标签相关（0.157 级），故 OOF 虚高；
- Public 回退：真 test 上 ensemble 预测质量低于 train 域（迁移），PL 微调放大 ensemble 偏差并**压缩模型自身多样性**（PL 版与 event256 原版相关性 0.994 vs SSL 版 0.9968），stack 后独立性下降 → Public -0.001；
- 这是本项目第 N 次 OOF 提升不迁移，但这次方向为**负**，说明伪标签微调对余弦指标是系统性陷阱（逼近集成 ≠ 逼近真值）。
- 教训：OOF 上接近 ensemble 的模型必然虚高；必须以多样性为护栏。

## 最终状态

- 最佳提交：**55601441（0.146）**，SSL 版 55666656（0.146）持平；
- SSL（无 PL）保留为 event 成员改进；PL 方向冻结。
