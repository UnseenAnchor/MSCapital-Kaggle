"""Multi-resolution v2+v3 self-anchor model. Proxy gate first; never submits."""
...
def batches(A2,A3,idx,y,bs,shuffle,seed,maxq=3):
 ...
 def work():
  try:
   for i in range(0,len(ix),bs):
    j=ix[i:i+bs];q.put((A2['market'][j],A2['tx'][j],A2['order'][j],A3['market'][j],A3['tx'][j],A3['order'][j],y[j]))
  finally:q.put(stop)
 ...
 while True:
  ...
  yield z
class Prep:
 def __init__(self,s2,s3):self.s=[(torch.tensor(a,device=DEVICE),torch.tensor(b,device=DEVICE)) for a,b in [s2['market'],s2['tx'],s2['order'],s3['market'],s3['tx'],s3['order']]]
 def one(self,x,i):
 ...
 def batch(self,b):return tuple(self.one(x,i) for i,x in enumerate(b[:6]))+(torch.from_numpy(b[6]).to(DEVICE),)
...
class Net(nn.Module):
 def __init__(self):
  super().__init__();self.e=nn.ModuleList([Stream(11,200),Stream(7,60),Stream(10,60),Stream(11,400),Stream(7,120),Stream(10,120)]);el=nn.TransformerEncoderLayer(D,4,D*4,.1,'gelu',batch_first=True,norm_first=True);self.cross=nn.TransformerEncoder(el,2);self.typ=nn.Parameter(torch.randn(1,6,D)*.02);self.h=nn.Sequential(nn.Linear(D*12,D*4),nn.GELU(),nn.Dropout(.1),nn.Linear(D*4,D*2),nn.GELU(),nn.Linear(D*2,1))
 def forward(self,*x):
