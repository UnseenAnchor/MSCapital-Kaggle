...
def loss_fn(p,y):
 mean=p.mean(1);pc=mean-mean.mean();yc=y-y.mean();cos=1-F.cosine_similarity(pc[None],yc[None],dim=1,eps=1e-8).mean();mse=F.smooth_l1_loss(p,y[:,None].expand_as(p));div=((p-p.mean(1,keepdim=True))**2).mean();return cos+.02*mse-.0002*div,cos,mse
@torch.no_grad()
...
def main():
 ...
 print(f'DEVICE={DEVICE} topn={TOPN} ens={N_ENS} params={sum(p.numel() for p in m.parameters())/1e6:.2f}M batch={BS} epochs={EPOCHS} train={len(tr)} valid={len(va)}',flush=True)
 for ep in range(1,EPOCHS+1):
  ...
  p=predict(m,Xt[va]);met=metrics(y[va],p,mo[va]);history.append(dict(epoch=ep,loss=tot/len(tr),lr=lr,**met));print('epoch',ep,history[-1],'sec',round(time.time()-st),flush=True)
  torch.save({'model':m.state_dict(),'cols':cols,'med':sc.med,'fac':sc.fac,'epoch':ep,'metrics':met,'topn':TOPN,'n_ens':N_ENS},f'output/realmlp_proxy_v3_ep{ep}.pt')
  if met['cosine']>best[0]:best=(met['cosine'],ep)
 em=RealMLP(TOPN).to(DEVICE);ema.apply_to(em);pm=predict(em,Xt[va],member=True);p=pm.mean(1);emet=metrics(y[va],p,mo[va]);print('EMA',emet,'member cosine',[round(cosine(y[va],pm[:,i]),6) for i in range(N_ENS)],flush=True)
 np.savez('output/realmlp_proxy_v3_oof.npz',sample_id=lab.sample_id.to_numpy()[va],target=y[va]/Y_SCALE,month=mo[va],prediction=p/Y_SCALE,members=pm/Y_SCALE)
 torch.save({'model':em.state_dict(),'cols':cols,'med':sc.med,'fac':sc.fac,'metrics':emet,'topn':TOPN,'n_ens':N_ENS,'epochs':EPOCHS},'output/realmlp_proxy_v3_ema.pt');json.dump({'best':best,'ema':emet,'history':history},open('output/realmlp_proxy_v3_metrics.json','w'),indent=2);print('best',best,'total_sec',time.time()-t0)
