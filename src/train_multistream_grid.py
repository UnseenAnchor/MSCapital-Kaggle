...
def main():
 random.seed(SEED);np.random.seed(SEED);torch.manual_seed(SEED);lab=pd.read_feather('data/train/label.feather').sort_values('sample_id');n=len(lab);A=arrays('train',n);mo=lab.month.to_numpy();y=lab.target.to_numpy(np.float32);tr=np.flatnonzero((mo>=TRAIN_START)&(mo<TRAIN_END));va=np.flatnonzero((mo>=TRAIN_END)&(mo<VALID_END));norm_idx=np.concatenate([tr,va]) if os.environ.get('NORM_WITH_VAL','0')=='1' and len(va) else tr;norm=norm_stats(A,norm_idx);print(f'norm stats rows={len(norm_idx)} train_only={len(norm_idx)==len(tr)}',flush=True);np.savez(f'{ROOT}/norm_stats_{OUT_PREFIX}.npz',**{f'{k}_{z}':v[i] for k,v in norm.items() for i,z in enumerate(['mean','std'])})
 sample_weights=None
 if DOMAIN_SAMPLING:
